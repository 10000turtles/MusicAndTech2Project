...
MidiFile::MidiFile(const MidiFile &other) { *this = other; }

MidiFile::MidiFile(MidiFile &&other) { *this = std::move(other); }
...
MidiFile &MidiFile::operator=(const MidiFile &other) {
  ...
  std::generate_n(
      std::back_inserter(m_events), other.m_events.size(),
...
//////////////////////////////
//
// MidiFile::readSmf -- Parse a Standard MIDI File and store its contents in the
// object.
//
...
bool MidiFile::readSmf(std::istream &input) {
  ...
  character = input.get();
  if (character == EOF) {
    std::cerr << "In file " << filename << ": unexpected end of file."
              << std::endl;
    ...
    std::cerr << "Expecting 'M' at first byte but got '" << (char)character
              << "'" << std::endl;
    m_rwstatus = false;
    return m_rwstatus;
  }
  ...
  if (character == EOF) {
    std::cerr << "In file " << filename << ": unexpected end of file."
              << std::endl;
    std::cerr << "Expecting 'T' at second byte, but found nothing."
              << std::endl;
    m_rwstatus = false;
    return m_rwstatus;
  } else if (character != 'T') {
    std::cerr << "File " << filename << " is not a MIDI file" << std::endl;
    std::cerr << "Expecting 'T' at second byte but got '" << (char)character
              << "'" << std::endl;
    m_rwstatus = false;
    return m_rwstatus;
  }
  ...
  if (character == EOF) {
    ...
    std::cerr << "File " << filename << " is not a MIDI file" << std::endl;
    std::cerr << "Expecting 'h' at third byte but got '" << (char)character
              << "'" << std::endl;
    m_rwstatus = false;
    return m_rwstatus;
  }
  ...
  if (character == EOF) {
    std::cerr << "In file " << filename << ": unexpected end of file."
              << std::endl;
    std::cerr << "Expecting 'd' at fourth byte, but found nothing."
              << std::endl;
    m_rwstatus = false;
    return m_rwstatus;
  } else if (character != 'd') {
    std::cerr << "File " << filename << " is not a MIDI file" << std::endl;
    std::cerr << "Expecting 'd' at fourth byte but got '" << (char)character
              << "'" << std::endl;
    m_rwstatus = false;
    return m_rwstatus;
  }
  ...
  if (longdata != 6) {
    std::cerr << "File " << filename << " is not a MIDI 1.0 Standard MIDI file."
              << std::endl;
  ...
  default:
    std::cerr << "Error: cannot handle a type-" << shortdata << " MIDI file"
              << std::endl;
    m_rwstatus = false;
    return m_rwstatus;
  }
  ...
  if (type == 0 && shortdata != 1) {
    std::cerr << "Error: Type 0 MIDI file can only contain one track"
              << std::endl;
  ...
  if (shortdata >= 0x8000) {
    ...
    // std::cerr << "SMPTE ticks: " << m_ticksPerQuarterNote << " ticks/sec" <<
    // std::endl; std::cerr << "SMPTE frames per second: " << framespersecond <<
    // std::endl; std::cerr << "SMPTE subframes per frame: " << subframes <<
    // std::endl;
  } else {
  ...
  // allows for reconstruction of the order when merging/splitting tracks
  // to/from a type-0 configuration.
  markSequence();
...
bool MidiFile::status(void) const { return m_rwstatus; }
...
MidiEventList &MidiFile::operator[](int aTrack) { return *m_events[aTrack]; }
...
int MidiFile::getNumTracks(void) const { return getTrackCount(); }

//
// Alias for getTrackCount()
//

int MidiFile::size(void) const { return getTrackCount(); }
...
int MidiFile::getTrackState(void) const { return m_theTrackState; }
...
void MidiFile::makeDeltaTicks(void) {
  ...
  for (i = 0; i < length; i++) {
    ...
    for (j = 1; j < (int)m_events[i]->size(); j++) {
      ...
      if (deltatick < 0) {
        std::cerr << "Error: negative delta tick value: " << deltatick
                  << std::endl
...
void MidiFile::deltaTicks(void) { makeDeltaTicks(); }
...
void MidiFile::absoluteTicks(void) { makeAbsoluteTicks(); }
...
int MidiFile::getTickState(void) const { return m_theTimeState; }
...
void MidiFile::doTimeAnalysis(void) { buildTimeMap(); }
...
int MidiFile::linkEventPairs(void) { return linkNotePairs(); }
...
const char *MidiFile::getFilename(void) const { return m_readFileName.c_str(); }
...
MidiEvent *MidiFile::addCopyright(int aTrack, int aTick,
...
MidiEvent *MidiFile::addTrackName(int aTrack, int aTick,
...
MidiEvent *MidiFile::addNoteOn(int aTrack, int aTick, int aChannel, int key,
                               int vel) {
  MidiEvent *me = new MidiEvent;
...
MidiEvent *MidiFile::addController(int aTrack, int aTick, int aChannel, int num,
                                   int value) {
  MidiEvent *me = new MidiEvent;
...
MidiEvent *MidiFile::addTimbre(int aTrack, int aTick, int aChannel,
                               int patchnum) {
...
MidiEvent *MidiFile::addPitchBend(int aTrack, int aTick, int aChannel,
                                  double amount) {
  m_timemapvalid = 0;
  amount += 1.0;
...
//   alteration of a note.  Default is 2.0 (meaning +/- 2 semitones from given
//   pitch). Fractional values are cents, so 2.5 means a range of two semitones
//   plus 50 cents, which is two semitones plus a quarter tone.
//

void MidiFile::setPitchBendRange(int aTrack, int aTick, int aChannel,
                                 double range) {
  if (range < 0.0) {
    range = -range;
  }
  if (range > 24.0) {
    std::cerr << "Warning: pitch bend range is too large: " << range
              << std::endl;
  ...
  addController(aTrack, aTick, aChannel, 6,
                irange); // coarse: number of semitones
  addController(aTrack, aTick, aChannel, 38,
                cents); // fine: cents (1/100ths of semitone)
}
...
MidiEvent *MidiFile::addSustain(int aTrack, int aTick, int aChannel,
                                int value) {
...
MidiEvent *MidiFile::addSustainPedal(int aTrack, int aTick, int aChannel,
                                     int value) {
...
int MidiFile::addTracks(int count) { return addTrack(count); }
...
void MidiFile::erase(void) { clear(); }
...
int MidiFile::getTPQ(void) const { return getTicksPerQuarterNote(); }
...
void MidiFile::setTPQ(int ticks) { setTicksPerQuarterNote(ticks); }
...
void MidiFile::setMillisecondTicks(void) { m_ticksPerQuarterNote = 0xE728; }
...
int MidiFile::extractMidiData(std::istream &input, std::vector<uchar> &array,
  ...
  if (byte < 0x80) {
    runningQ = 1;
    if (runningCommand == 0) {
      std::cerr << "Error: running command with no previous command"
                << std::endl;
      return 0;
    }
    if (runningCommand >= 0xf0) {
      std::cerr << "Error: running status not permitted with meta and sysex"
                << " event." << std::endl;
      std::cerr << "Byte is 0x" << std::hex << (int)byte << std::dec
                << std::endl;
      return 0;
    }
  } else {
...
std::ostream &MidiFile::writeLittleEndianUShort(std::ostream &out,
                                                ushort value) {
  union {
    char bytes[2];
    ushort us;
...
std::ostream &MidiFile::writeLittleEndianDouble(std::ostream &out,
                                                double value) {
  union {
    char bytes[8];
    double d;
